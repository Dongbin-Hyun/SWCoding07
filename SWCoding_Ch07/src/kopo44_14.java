...
public class kopo44_14 {
	
	static ArrayList <kopo44_13> kopo44_ArrayOneRec = new ArrayList <kopo44_13>();//클래스의 ArrayLlist ArrayOneRec을 생성.
	...
	static int kopo44_pcnt = 0;//전역변수 sumavg을 정의하고 값을 0으로 초기화.
	static int kopo44_cnt = 0;//전역변수 cnt을 정의하고 값을 0으로 초기화.
	static int kopo44_pagesumkor = 0;// 숫자형전역수 pagekorsum을 정의하고 값을 0으로 초기화.
	static int kopo44_pagesumeng = 0;// 숫자형전역변수 pageengsum을 정의하고 값을 0으로 초기화.
	static int kopo44_pagesummat = 0;// 숫자형전역변수 pagematsum을 정의하고 값을 0으로 초기화.
	static int kopo44_pagesumsum = 0;// 숫자형전역변수 pagetotalsum을 정의하고 값을 0으로 초기화.
	static int kopo44_pagesumavg = 0;// 실수형전역변수 pageavg을 정의하고 값을 0으로 초기화.
	...
	public static void main(String[] args) {//메인으로부터 프로그램시작
		...
		kopo44_pcnt = 1;
		for(int kopo44_i = 0; kopo44_i < kopo44_ArrayOneRec.size(); kopo44_i++) {//for반복문은 i는 0부터 ArrayOneRec의 크기까지 i를 1씩 증가시키면서 수행.
			...
			kopo44_ItemPrint(kopo44_i);//ItemPrint함수를 파라미터로 i로 주고 실행.
	...
	private static void kopo44_TailPrint() {//TailPrint함수를 생성.
		// TODO Auto-generated method stub
		System.out.printf("=========================================\n");//*선과 줄바꿈을 화면에 출력.
		System.out.printf("현재페이지\n");//현재페이지와 줄바꿈을 화면에 출력
		System.out.printf("국어합계 %7d  국어평균: %6.2f\n", kopo44_pagesumkor, kopo44_pagesumkor / 30.);//변수 pagesumkor과 cnt로 나눈 sumpagekor를 화면에 출력.
		System.out.printf("영어합계 %7d  영어평균: %6.2f\n", kopo44_pagesumeng, kopo44_pagesumeng / 30.);//변수 pagesumeng과 cnt로 나눈 sumpageeng를 화면에 출력.
		System.out.printf("수학합계 %7d  수학평균: %6.2f\n", kopo44_pagesummat, kopo44_pagesummat / 30.);//변수 pagesummat과 cnt로 나눈 sumpagemat를 화면에 출력.
		System.out.printf("반평균합계 %5d    반평균:  %5.2f\n", kopo44_pagesumavg, kopo44_pagesumavg / 30.);//변수 pagesumavg과 cnt로 나눈 sumpageavg를 화면에 출력.
		System.out.printf("=========================================\n");//*선과 줄바꿈을 화면에 출력.
		System.out.printf("누적페이지\n");//누적페이지와 줄바꿈을 화면에 출력
		System.out.printf("국어합계 %7d  국어평균: %6.2f\n", kopo44_sumkor, kopo44_sumkor / (double)kopo44_cnt);//변수 sumkor과 cnt로 나눈 sumkor를 화면에 출력.
		System.out.printf("영어합계 %7d  영어평균: %6.2f\n", kopo44_sumeng, kopo44_sumeng / (double)kopo44_cnt);//변수 sumeng과 cnt로 나눈 sumeng를 화면에 출력.
		System.out.printf("수학합계 %7d  수학평균: %6.2f\n", kopo44_summat, kopo44_summat / (double)kopo44_cnt);//변수 summat과 cnt로 나눈 summat를 화면에 출력.
		System.out.printf("반평균합계 %5d    반평균:  %5.2f\n", kopo44_sumavg, kopo44_sumavg / (double)kopo44_cnt);//변수 sumavg과 cnt로 나눈 sumavg를 화면에 출력.
		System.out.println();//줄바꿈을 화면에 출력.

		
	}

	private static void kopo44_ItemPrint(int kopo44_i) {//ItemPring함수를 생성하고 파라미터로 숫자형 변수 i를 줌
		// TODO Auto-generated method stub
		kopo44_13 kopo44_rec;//P9OneRec 클래스를 받아서 객체를 만듦.
		
		kopo44_rec = kopo44_ArrayOneRec.get(kopo44_i);//인스턴스를 생성하여서 객체에 연결.
	...
	private static void kopo44_HeaderPrint() {//HeaderPring함수를 생성.
		...
		System.out.printf("%16s\n","성적집계표");//성적집계표와 줄바꿈을 화면에 출력
		System.out.printf("PAGE : %d  출력일자 : %s\n", kopo44_pcnt, kopo44_sf.format(kopo44_d));//날짜를 받아온 변수 d에 아까 지정한 SimpleDateFormat으로 yyyy.MM.dd HH:mm:ss 형태로 출력.
		System.out.printf("=========================================\n");//*선과 줄바꿈을 화면에 출력.
		System.out.printf("%2s%4s %3s %2s %2s %2s   %2s\n", "번호", "이름", "국어", "영어", "수학", "합계", "평균");//Header를 화면에 출력.
		System.out.printf("=========================================\n");//*선과 줄바꿈을 화면에 출력.
	}
	...
	private static void kopo44_dataSet() {//dataSetPrint함수를 생성.
		...
		for (int kopo44_i = 0; kopo44_i < kopo44_iPerson; kopo44_i++) {//for반복문으로 i가 0부터 iPerson보다 작을때까지 1씩 증가시키면서 수행.
			...
			kopo44_ArrayOneRec.add(new kopo44_13(kopo44_i + 1, kopo44_name, kopo44_kor, kopo44_eng, kopo44_mat));//ArrayList에 P9OneRec 함수를 통해서 만들어진 값을 저장.
		}
	}
}
